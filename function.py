from infogram_api_3 import Infogram
...
INFOGRAM_API_KEY = os.getenv('INFOGRAM_API_KEY')
INFOGRAM_API_SECRET = os.getenv('INFOGRAM_API_SECRET')

def get_cached_results(key='latest.json'):
    params = {
...
def parse_sd(xml):
    ...
    try:
        old_results = json.loads(get_cached_results())
